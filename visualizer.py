...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256, i2=None):
    ...
    for label, im_data in visuals.items():
        ...
        if i2 is not None:
            image_name = '%s/%s_%s.png' % (label, name, i2)
        # pdb.set_trace()
...
class Visualizer():
    ...
    def __init__(self, name, log_path, opt):
        ...
        if self.use_html:  # create an HTML object at <checkpoints_dir>/web/; images will be saved under <checkpoints_dir>/web/images/
            self.web_dir = os.path.join(log_path, name, 'web')

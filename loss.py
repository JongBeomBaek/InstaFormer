...
def model_forward(inputs, model, num_box=-1, task=I2I, feat_layers=[]): #i2i==False: reconstruction task
    if task == I2I:
        ...
        rand_style = torch.randn(inputs['A'].shape[0], 8, 1, 1)
    ...
    fake_box = model['Decoder'](aggregated_box, inputs['A_box']) if task == I2I else None
...
# def compute_instNCE_loss(feat_q, feat_k, model, criterionNCE, train_cfg):
#     n_layers = len(train_cfg.model.feat_layers)

#     if train_cfg.num_box > 0:
#         box_info = box_info_A  if fromBox == 'A' else box_info_B
#         if fromBox =='A':
#             feat_q_none, feat_q_all, box_index_list = netG(tgt, box_info, train_cfg.model.feat_layers, fake_B_box_list, encode_only=True, feat_K=False, feat_Q=True)
#         elif fromBox =='B':
#             feat_q_none, feat_q_all, box_index_list = netG(tgt, box_info=None, layers=train_cfg.model.feat_layers, fake_box_list=idt_B_box_list, encode_only=True, feat_K=False, feat_Q=True)

#     else:
#         pass

#     if self.opt.flip_equivariance and flipped_for_equivariance:
#         feat_q_all = [torch.flip(fq, [3]) for fq in feat_q_all]

#     if train_cfg.num_box > 0:
#             feat_k_none, feat_k = netG(src, box_info, train_cfg.model.feat_layers, encode_only=True, feat_K=True,feat_Q=False)
#     else:
#         pass
...
#     feat_q_all = torch.split(feat_q_all,1,dim=0) 
#     feat_k = torch.split(feat_k,1,dim=0)

#     feat_k_pool, sample_ids = netF_inst(feat_k, 64, None)  
#     feat_q_pool_all, _ = netF_inst(feat_q_all, 64, sample_ids)

#     feat_k_list=[]
#     for i in range(train_cfg.batch_size// max(len(self.opt.gpu_ids), 1)):
#         feat_k_list.extend(feat_k_pool[0:box_index_list[i]]) 

#         del feat_k_pool[0:train_cfg.num_box]

#     feat_k_pool = feat_k_list
#     feat_q_list=[]
#     for i in range(train_cfg.batch_size// max(len(self.opt.gpu_ids), 1)):
#         feat_q_list.extend(feat_q_pool_all[0:box_index_list[i]])
#         del feat_q_pool_all[0:train_cfg.num_box]
#     feat_q_pool = feat_q_list

#     total_nce_loss = 0.0

#     for f_q, f_k, crit, box_idx in zip(feat_q_pool, feat_k_pool, instNCE, range(sum(box_index_list))):
#         if torch.eq(f_k, torch.zeros_like(f_k)).sum()>int(64*64):
#             if box_idx == 0:
#                 return None
#             else:
#                 return total_nce_loss / (box_idx)
#         loss = crit(f_q, f_k) * train_cfg.w_NCE
